...
describe('userSearchesNotFound', () => {
  ...
  it('should show an error banner when there is no matching user', async () => {
    page
      .fillIn('search-input', 'doesntexist')
      .clickButton('Go!')

    await page.waitUntil(() => page.findWrapperForClickLink('Dismiss'))
    expect(page.content()).to.match(/Not Found/)
    page.clickLink('Dismiss')
    expect(page.content()).not.to.match(/Not Found/)
  })

  it('should show an error banner when there is no matching repository', async () => {
    page
