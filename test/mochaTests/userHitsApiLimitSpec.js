...
describe('userHitsApiLimit', () => {
  ...
  it('should show an error banner when API limit is reached when searching for a user', async () => {
    page
      .fillIn('search-input', 'IntrepidPursuits')
      .clickButton('Go!')

    await page.waitUntil(() => page.findWrapperForClickLink('Dismiss'))
    expect(page.content()).to.match(/API rate limit exceeded/)
    page.clickLink('Dismiss')
    expect(page.content()).not.to.match(/API rate limit exceeded/)
  })

  it('should show an error banner when API limit is reached when searching for a repository', async () => {
    page
