...
describe('userVisitsUserPage', () => {
  ...
  describe('user with starred repositories', () => {
    ...
    it('should show the user information', asyncTest(async () => {
      expect(page.content()).toMatch(/Loading davidrf's profile.../)
      expect(page.content()).not.toMatch(/davidrf \(David Rodriguez Fuentes\)/)
      await page.waitUntil(() => page.contentMatches(/davidrf \(David Rodriguez Fuentes\)/))
    }))

    it('should show the user\'s starred repositories', asyncTest(async () => {
      await page.waitUntil(() => page.contentMatches(/davidrf \(David Rodriguez Fuentes\)/))
      expect(page.content()).toMatch(/Loading davidrf's starred.../)
      expect(page.content()).not.toMatch(/elixir by elixir-lang/)
      await page.waitUntil(() => page.contentMatches(/elixir by elixir-lang/))
    }))

    it('should show more user starred repositories when clicking the \'Load More\' button', asyncTest(async () => {
      await page.waitUntil(() => page.findWrapperForClickButton('Load More').exists())
      page.clickButton('Load More')
      expect(page.content()).toMatch(/Loading.../)
      expect(page.content()).not.toMatch(/curriculum by davidrf/)
      await page.waitUntil(() => page.contentMatches(/curriculum by davidrf/))
      expect(page.findWrapperForClickButton('Load More').exists()).toEqual(false)
    }))

    it('should allow navigation to the user page of a starred repository author', asyncTest(async () => {
      await page.waitUntil(() => page.findWrapperForClickLink('IntrepidPursuits').exists())
      page.clickLink('IntrepidPursuits')
      expect(page.currentPath()).toEqual('/IntrepidPursuits')
    }))

    it('should allow navigation to the repository page of a starred repository', asyncTest(async () => {
  ...
  describe('user with no starred repositories', () => {
    ...
    it('should show the user\'s starred repositories', asyncTest(async () => {
      expect(page.content()).not.toMatch(/Nothing here!/)
